...
const handler = async (event, context) => {
  console.log('🚀 Weather cron job triggered at', new Date().toISOString());
  ...
  try {
    ...
    console.error('❌ Error in weather cron job:', error.message);
    return {
      statusCode: 500,
      body: JSON.stringify({
        success: false,
...
// Export with Netlify schedule (runs every hour)
